...
void		ft_add_room(t_list **head, char *str)
{
	t_room *room;
	// printf("start add room\n");
	room = ft_create_room(str);
	printf("%d\n", room->visited);
	ft_lstadd(head, ft_lstnew(room, sizeof(room)));
	// printf("new room name:%s visited:%d\n", room->info[0], room->visited);
	// ft_lstadd(&head, ft_lstnew(room, sizeof(room)));
	// printf("room created\n");
	...
	// {
	// 	printf("first room\n");
	// 	*head = ft_lstnew(room, sizeof(room));
	// }
	// else
	// {
		// printf("%s next %s\n", ((t_room*)head->content)->info[0], ((t_room*)head->next->content)->info[0]);
	// }
	// free(room);
}
...
void		ft_handle_input(t_main *data)
{
	...
	while(room_head)
	{
		printf("%d\n", ((t_room*)room_head->content)->visited);
